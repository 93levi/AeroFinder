...
export default function RentalModal({ rentalId, onClose }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setRental({ ...data, id: Number(rentalId) });
        if (ratingsRes && ratingsRes.ok) setRatings((await ratingsRes.json()) ?? []);
      } catch (err) {
  ...
  async function handleSubmitRating() {
    ...
    try {
      ...
      const r2 = await fetch(`${API}/ratings/rentals/${rentalId}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
  ...
  return (
    <div className="rental-modal-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="rental-modal">

        <div className="rp-header">
          <button className="rp-back" onClick={onClose}>Back</button>
        ...
        {rental && !loading && (
          <div className="rp-body">
            ...
            <div className="rp-stats">
              <div className="rp-stat"><span className="rp-stat-val">{rental.bedrooms}</span><span className="rp-stat-lbl">Bed</span></div>
              <div className="rp-stat"><span className="rp-stat-val">{rental.bathrooms}</span><span className="rp-stat-lbl">Bath</span></div>
              <div className="rp-stat"><span className="rp-stat-val">{rental.parkingSpaces}</span><span className="rp-stat-lbl">Parking</span></div>
            </div>

            <div className="rp-columns">
              <div className="rp-main">
                {rental.description && (
                  <p className="rp-description">{rental.description.replace(/<[^>]*>/g, ' ').trim()}</p>
                )}
                      ...
                      {!ratingSuccess ? (
                        <>
                          ...
                          <div className="rp-star-row">
                            {[1,2,3,4,5].map(n => (
                              <button key={n} className={`rp-star-btn ${userRating >= n ? 'rp-star-active' : ''}`} onClick={() => setUserRating(n)}>
                                {userRating >= n ? '★' : '☆'}
                              </button>
                            ))}
                          </div>
                          {ratingError && <p className="rp-rating-error">{ratingError}</p>}
                          <button className="rp-rating-submit" onClick={handleSubmitRating} disabled={!userRating || ratingSubmitting}>
              ...
              {rental.latitude && rental.longitude && (
                <div className="rp-map-col">
                  <Map height={380} defaultCenter={[rental.latitude, rental.longitude]} defaultZoom={14} attribution={false}>
